...
impl WooCommerceProduct {
    ...
    pub fn merge(&self, other: &WooCommerceProduct) -> WooCommerceProduct {
        ...
        if !all_stock_status_type.contains(
            &self
                .stock_status
                .as_deref()
                .unwrap_or("")
    ...
    pub fn get_changes(&self, other: &WooCommerceProduct) -> Vec<String> {
        ...
        if !other.categories.is_empty() {
            if self.categories.iter().map(|c| c.id).collect::<Vec<_>>() != other.categories.iter().map(|c| c.id).collect::<Vec<_>>() {
                changes.push("categories".to_string());
            }
        }

        if !other.tags.is_empty() {
            if self.tags.iter().map(|c| c.id).collect::<Vec<_>>() != other.tags.iter().map(|c| c.id).collect::<Vec<_>>() {
...
impl ProductVariation {
    ...
    pub fn set_parent(&mut self, parent: &String) {
        ...
        println!(
            "\x1b[38;5;220mUpdated child with ID {} and SKU {} parent ID to {}\x1b[0m",
            self.id, self.sku, parent
        );
    }
...
pub fn woo_product_builder(
    ...
    if !parent.is_empty() {
        ...
        // check if type is in array of simple, grouped, external and variable
        if !["simple", "grouped", "external", "variable", "variation"]
            .contains(&type_.to_lowercase().as_str())
        {
            type_ = String::new(); // set to empty string if not valid
        }
    }
    let manage_stock_value = get_value("manage_stock");
    let mut manage_stock = None;
    let all_manage_stock_type = vec!["yes", "true", "1"];
    // Check if manage_stock is valid
    if all_manage_stock_type.contains(&manage_stock_value.as_str()) {
    ...
    println!(
        "\x1b[38;5;43mShipping Class for Product ID: {}, SKU: {}: {}\x1b[0m",
        id,
        sku,
        shipping_class_option.clone().unwrap_or_default()
    );
    ...
    Ok(WooCommerceProduct {
        ...
        attributes,
        manage_stock: if let Some(manage_stock) = manage_stock {
            Some(manage_stock)
        } else if stock_quantity.is_some() {
            Some(true)
        } else {
            None
        },
...
pub fn build_product_dimensions(product: &HashMap<String, String>) -> Option<ProductDimension> {
    let get_value = |key: &str| -> String {
        product
            .get(key)
            .unwrap_or(&"".to_string())
    ...
    <T as FromStr>::Err: Display,
{
    ...
    match value {
        ...
        serde_json::Value::Number(n) => {
            ...
            T::from_str(&num_str)
                .map_err(|e| D::Error::custom(format!("Failed to parse number: {}", e)))
        }
...
fn deserialize_optional_bool_none_if_false<'de, D>(
    deserializer: D,
